...
from utils.store_results import update_results_file
...
class ExperimentBuilder(object):
        ...
        for key, value in zip(list(summary_losses.keys()), list(summary_losses.values())):
            if "loss" in key or "accuracy" in key:
                if "importance" in key:
                    key = "loss_weight_" + key.split("_")[-1]
                if "loss_weight_std" in key or "loss_weight_mean" in key or "val_loss" in key:
                    pass
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        PROXY=10000 # self.args.num_evaluation_tasks
        ...
        for idx, model_idx in enumerate(top_n_idx):
            ...
            with tqdm.tqdm(total=int(PROXY / self.args.batch_size)) as pbar_test:
                for sample_idx, test_sample in enumerate(
                        self.data.get_test_batches(total_batches=int(PROXY / self.args.batch_size),
                                                   augment_images=False)):
                    # print(test_sample[0].shape, test_sample[1].shape, test_sample[2].shape, test_sample[3].shape)
        ...
        # print(per_batch_max.shape) # 496, 75
        # print(np.equal(per_batch_targets, per_batch_max).shape) # 496, 75
        accuracy = np.mean(np.equal(per_batch_targets, per_batch_max), axis=1)
        # print(accuracy.shape, '496')
...
#             if self.args.evaluate_on_test_set_only == False:
